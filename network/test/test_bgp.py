...
class TestBGP(TestORCA):
    ...
    def test_complete_bgp_setup(self):
        ...
        intf_request_body = {
            "mgt_ip": device_ip_2,
            "name": ether_name,
            "ip_address": f"{neighbor_ip_1}/{prefix_len}",
        }
        self.assert_with_timeout_retry(
            lambda path, data: self.put_req(path, data),
            "device_interface_list",
        ...
        # verifying the ip_address value
        response = self.get_req("subinterface", {"mgt_ip": device_ip_2, "name": ether_name})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response_body = response.json()
        if isinstance(response_body, list):
            self.assertTrue(any([i["ip_address"] == neighbor_ip_1 for i in response_body]))
        else:
            self.assertEqual(response_body["ip_address"], neighbor_ip_1)
        ...
        # adding bgp address Family to device 1
        bgp_af = {
            "mgt_ip": device_ip_1,
            "vrf_name": "default",
            "afi_safi": "ipv4_unicast",
        }
        response = self.put_req("bgp_af", bgp_af)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.get_req("bgp_af", {
            "mgt_ip": device_ip_1, "local_asn": device_1_asn, "afi_safi": "ipv4_unicast"
        })
        self.assertEqual(response.json()["afi_safi"], "ipv4_unicast")
        self.assertEqual(response.json()["vrf_name"], "default")

        #  adding bgp address Family to device 2
        bgp_af = {
            "mgt_ip": device_ip_2,
            "vrf_name": "default",
            "afi_safi": "ipv4_unicast",
        ...
        response = self.get_req("bgp_af", {
            "mgt_ip": device_ip_2, "local_asn": device_2_asn, "afi_safi": "ipv4_unicast"
        })
        self.assertEqual(response.json()["afi_safi"], "ipv4_unicast")
        self.assertEqual(response.json()["vrf_name"], "default")

        # adding bgp af network to device 1
        bgp_net = {
            "mgt_ip": device_ip_1,
            "vrf_name": "default",
            "afi_safi": "ipv4_unicast",
        ...
        response = self.put_req("bgp_af_network", bgp_net)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(response.json()["afi_safi"], "ipv4_unicast")
        self.assertEqual(response.json()["vrf_name"], "default")
        self.assertEqual(response.json()["ip_prefix"], "244.178.44.111/24")

        # adding bgp af network to device 2
        bgp_net = {
            "mgt_ip": device_ip_2,
            "vrf_name": "default",
            "afi_safi": "ipv4_unicast",
        ...
        # adding bgp af aggregate address to device 1
        bgp_aggr = {
            "mgt_ip": device_ip_1,
            "vrf_name": "default",
            "afi_safi": "ipv4_unicast",
        ...
        response = self.put_req("bgp_af_aggregate_addr", bgp_aggr)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(response.json()["afi_safi"], "ipv4_unicast")
        self.assertEqual(response.json()["vrf_name"], "default")
        self.assertEqual(response.json()["ip_prefix"], "192.84.2.178/24")

        # adding bgp af aggregate address to device 2
        bgp_aggr = {
            "mgt_ip": device_ip_2,
            "vrf_name": "default",
            "afi_safi": "ipv4_unicast",
        ...
        # verify bgp af aggregate address
        response = self.get_req("bgp_af_aggregate_addr", {
            "mgt_ip": device_ip_2, "local_asn": device_2_asn, "afi_safi": "ipv4_unicast"
        })
        self.assertEqual(response.json()["afi_safi"], "ipv4_unicast")
        self.assertEqual(response.json()["vrf_name"], "default")
        self.assertEqual(response.json()["ip_prefix"], "1.1.1.1/24")

        # add bgp neighbor config remote as pointing device 1 and local asn pointing to device 2
        nbr_req = {
            "mgt_ip": device_ip_1,
            "remote_asn": device_1_asn,
            "vrf_name": "default",
            "local_asn": device_2_asn,
            "neighbor_ip": neighbor_ip_1
        }
        ...
        response = self.get_req("bgp_nbr", {"mgt_ip": device_ip_1, "neighbor_ip": neighbor_ip_1})
        self.assertEqual(response.json()["remote_asn"], device_1_asn)
        self.assertEqual(response.json()["local_asn"], device_2_asn)
        self.assertEqual(response.json()["vrf_name"], "default")
        self.assertEqual(response.json()["neighbor_ip"], neighbor_ip_1)

        # add bgp neighbor af to device 1
        ngr_af_body = {
            "mgt_ip": device_ip_1,
            "afi_safi": "ipv4_unicast",
            "vrf_name": "default",
        ...
        response = self.put_req("bgp_nbr_af", ngr_af_body)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.get_req("bgp_nbr_af", ngr_af_body)
        self.assertEqual(response.json()["afi_safi"], "ipv4_unicast")
        self.assertEqual(response.json()["vrf_name"], "default")
        self.assertEqual(response.json()["neighbor_ip"], "1.1.1.1")

        # validating connections

        # get bgp neighbor device 1 is device 2 bgp using local asn
        ...
        # get bgp neighbor device 1 is device 1 bgp using remote asn
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response_body = response.json()
        self.assertTrue(any([i["local_asn"] == device_1_asn for i in response_body]))
        self.assertTrue(any([i["router_id"] == device_ip_1 for i in response_body]))

        # get subinterfaces of bgp neighbor device 1

        response = self.get_req("bgp_nbr_subinterface", {
            "mgt_ip": device_ip_1, "neighbor_ip": neighbor_ip_1,
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response_body = response.json()
        if isinstance(response_body, list):
        ...
        # clean up
        response = self.del_req("subinterface", {"mgt_ip": device_ip_1, "name": ether_name})
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.get_req("subinterface", {"mgt_ip": device_ip_1, "name": ether_name})
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

        # removing bgp neighbor af of device 1
        response = self.del_req("bgp_nbr_af", {
            "mgt_ip": device_ip_1, "neighbor_ip": neighbor_ip_1
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # removing bgp neighbor of device 1
        response = self.del_req("bgp_nbr", nbr_req)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # removing bgp af of device 1
        response = self.del_req("bgp_af", {
            "mgt_ip": device_ip_1, "afi_safi": "ipv4_unicast", "local_asn": device_1_asn
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # removing bgp af of device 2
        response = self.del_req("bgp_af", {
            "mgt_ip": device_ip_2, "afi_safi": "ipv4_unicast", "local_asn": device_2_asn
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # removing bgp af network of device 1
        response = self.del_req("bgp_af_network", {
            "mgt_ip": device_ip_1, "afi_safi": "ipv4_unicast", "local_asn": device_1_asn
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # removing bgp af network of device 2
        response = self.del_req("bgp_af_network", {
            "mgt_ip": device_ip_2, "afi_safi": "ipv4_unicast", "local_asn": device_2_asn
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # removing bgp af aggregate address of device 1
        response = self.del_req("bgp_af_aggregate_addr", {
            "mgt_ip": device_ip_2, "afi_safi": "ipv4_unicast", "local_asn": device_1_asn
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # removing bgp af aggregate address of device 2
        response = self.del_req("bgp_af_aggregate_addr", {
            "mgt_ip": device_ip_1, "afi_safi": "ipv4_unicast", "local_asn": device_2_asn
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
