...
def device_interfaces_list(request):
    ...
    if request.method == 'GET':
        ...
        return Response({"status": "Error occurred while applying config."},
                                status=status.HTTP_500_INTERNAL_SERVER_ERROR) if not data else Response({"status": "Config Successful"},
                                status=status.HTTP_200_OK)
    else:
        return Response({"status": f"Unknown request method {request.method}"},
                                    status=status.HTTP_400_BAD_REQUEST)
